/*
  ...
  femto/src/dcx_cc/index_tool_support_fasta.c
*/
...
// Starting at start_offset in data, look for a FASTA sequence.
// Determine the end of the FASTA sequence.
// Store all of that in current_name and current_sequence.
...
int parse_data(const unsigned char* data, size_t data_len,
               ...
               size_t* next_offset)
{
  const char* dch = (const char*) data;
  size_t data_start, data_end;
  size_t hdr_start, hdr_end;
  size_t trailer_end;
  size_t nchars = 0;
  ...
  *next_offset = cur_offset;
  ...
  if( dch[cur_offset] != '>') {
    fprintf(stderr, "Warning: bad FASTA format in %s\n", path);
    // Scan forward until we get to the '>'.
    while( cur_offset < data_len && dch[cur_offset] != '>' ) cur_offset++;
  }
  ...
  hdr_start = cur_offset + 1; // don't include the >.

  // Find the end of the header string.
  for( hdr_end = hdr_start; hdr_end < data_len; hdr_end++ ) {
    if( dch[hdr_end] == '\n' ) break;
  }
  
  current_name = malloc(hdr_end - hdr_start + 1);
  if( ! current_name ) return -400;

  memcpy(current_name, &dch[hdr_start], hdr_end - hdr_start);
  current_name[hdr_end - hdr_start] = '\0';
  // OK, now 
  //printf("Got header %s\n", &data[hdr_start]);

  data_start = hdr_end + 1; // do not include newline.

  // Find the end of the region!
  // We take out the spaces and uppercase everything.
  nchars = 0;
  c = '\n';
  for( data_end = data_start; data_end < data_len; data_end++ ) {
    lastc = c;
    c = dch[data_end];
    if( lastc == '\n' && c == '>' ) break;
    if( !isspace(c) ) nchars++;
  ...
  if( doc_data ) {
    current_sequence = malloc(nchars + 1);
    if( ! current_sequence ) return -401;
    nchars = 0;
    c = '\n';
    for( data_end = data_start; data_end < data_len; data_end++ ) { 
      lastc = c;
      ...
      if( lastc == '\n' && c == '>' ) break;
      if( !isspace(c) ) {
        current_sequence[nchars++] = toupper(c);
      }
    }

    current_sequence[nchars] = '\0'; // just for the hell of it, really.
  }

  trailer_end = data_end;


  *info_len = hdr_end - hdr_start;
  *info_data = (unsigned char*) current_name;
  *doc_len = nchars;
  if( doc_data ) *doc_data = (unsigned char*) current_sequence;
  *next_offset = trailer_end;
...
{
  fprintf(out, "[fasta-file] [fasta-file] ...\n");
}
